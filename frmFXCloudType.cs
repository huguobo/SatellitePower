    ...
    public partial class frmFXCloudType : Form
    {
        ...
        public void HuiTu(Bitmap p, int k)
        {
            ...
            else
            {
                for (int i = 0; i < w1; i++)
                {
                    for (int j = 0; j < h1; j++)
                    {
                        Color wxc = Color.FromArgb(XX[i * h1 + j, 1], XX[i * h1 + j, 1], XX[i * h1 + j, 1]);//设置为1通道原色
                        p.SetPixel(i, j, wxc);
                    }
                }
            }
        ...
        public void SeBiao(Graphics gra)
        {
            ...
            for (int i = 0; i < kkk; i++)
            {
                sMsg = qzhushi[i];
                sCol = qcolor[i];
                Point sbP = new Point(100 + 100 * i, 15);
                ...
                gra.DrawString(sMsg, fnt, Brushes.Black, 140 + 100 * i, 15);
            }
            Point p99 = new Point(100 + 100 * kkk, 15);
            Point p999 = new Point(100 + 100 * (kkk + 1), 15);
            ...
            string s99 = "过渡云系";
            string s999 = "不可辨云系";
            gra.DrawString(s99, fnt, Brushes.Black, 140 + 100 * kkk, 15);
            gra.DrawString(s999, fnt, Brushes.Black, 140 + 100 * (kkk + 1), 15);
        }
        ...
        private void frmFXCloudType_Load(object sender, EventArgs e)
        {
            ...
            HuiTu(pic, 1);//聚类分析图
            HuiTu(pic0, 0);//原图
            Bitmap newpic = new Bitmap(pic, pbxMain.Width, pbxMain.Height);
            Bitmap newpic0 = new Bitmap(pic0, pbxYuan.Width, pbxYuan.Height);
