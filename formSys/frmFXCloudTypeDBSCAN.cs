...
namespace SatellitePower.formSys
{
    public partial class frmFXCloudTypeDBSCAN : Form
    {
        ...
        public void init()
        {
            ...
            p1.X = (int)Math.Round(x11, 0);
            p1.Y = (int)Math.Round(y22, 0);
            p2.X = (int)Math.Round(x22, 0);
            p2.Y = (int)Math.Round(y11, 0);
            ...
            for (int i = 0; i < w1; i++)
            {
                for (int j = 0; j < h1; j++)
                {
                    hd[i, j] = svu_IR1.bVissData[(y1 + j) * w + (x1 + i)];
                    if (hd[i, j] > 150)//有效样本点
                    {
                        data_id[i, j] = 0;
                        data_type[i, j] = 0;
        ...
        {
            dbscan_pbar.Visible = true;
            dbscan_label.Text = "正在生成...";
        ...
        public void setMax(int iMax)
        {
            ...
            if (this.dbscan_pbar.Control.InvokeRequired)
            {
        ...
        public void onMakeDataCompleted()
        {
            ...
            else
            {
                ...
                dbscan_pbar.Visible = false;
                dbscan_label.Text = "生成成功";
            }
        }

        private void frmFXCloudTypeDBSCAN_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (makeThread != null)
            {
                makeThread.Abort();
